...
// PUT /api/meeting-rooms/:id – modifier une salle (admin)
...
// DELETE /api/meeting-rooms/:id – désactiver une salle (soft delete)
...
// GET /api/meeting-rooms/:roomId/bookings – réservations d'une salle (avec détails)
router.get('/:roomId/bookings', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT b.*, s.full_name as booked_by_name 
       FROM room_bookings b
       LEFT JOIN staff s ON b.booked_by = s.id
...
router.get('/bookings/user/:userId', async (req, res) => {
  try {
    ...
    console.error('GET /meeting-rooms/bookings/user/:userId error:', err);
    res.status(500).json({ error: err.message });
  }
});

// POST /api/meeting-rooms/book – réserver une salle (ou réunion à distance)
router.post('/book', async (req, res) => {
  try {
    const { 
      room_id, booked_by, booked_by_name, title, description, 
      date, start_time, end_time, is_remote 
    } = req.body;
    ...
    // Si réunion physique, il faut une salle
    if (!is_remote && !room_id) {
      return res.status(400).json({ error: 'Choisissez une salle ou une réunion à distance' });
    }

    // Vérifier les conflits pour les réunions physiques
    if (!is_remote && room_id) {
    ...
    // Générer un lien de réunion Jitsi si c'est une réunion à distance
    let meeting_link = null;
    if (is_remote) {
      // Génération d'un ID unique pour la réunion Jitsi
    ...
    console.error('POST /meeting-rooms/book error:', err);
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/booking/:id', async (req, res) => {
  try {
    ...
    console.error('DELETE /meeting-rooms/booking/:id error:', err);
    res.status(500).json({ error: err.message });
  }
});
